...
def changeXPRFileWithParams(params, xpr_file):
    with open(xpr_file, 'r+') as f:
        ...
        data = []
        for param in params:
            regex = re.compile(param["key"] + r"\s*,\s*([-\d]+)",re.IGNORECASE)
            match_obj = re.search(regex, content)
            if not match_obj:
                print("XPR file does not include {} parameter".format(param["key"]))
                return [{"status":False}]
...
def getXMD3DData(xmd_file_path):
    (offset, samples, Views, Views2, Slices, Echoes, Experiments, Nex, Chennels, datasize) = Read_XMD(xmd_file_path)
...
def findBestChannel(xmd_dir):
    ...
    for xmd_file in os.listdir(xmd_dir):
        ...
        adc = np.max(np.array([np.max(np.real(s_3D[0])),np.max(np.imag(s_3D[0]))]))
    ...
    for c in range(len(s_3D)):
        adc = np.max(np.array([np.max(np.real(s_3D[c])),np.max(np.imag(s_3D[c]))]))
        if max_adc < adc:
            max_adc = adc
            channel = c
...
if __name__ == "__main__":
    print(changeXPRFileWithParams([{"key":"G3","value":12},{"key":"NO_VIEWS","value": 0},{"key":"G2", "value":0}],"XGYMR.XPR"))
